...
The project includes a comprehensive test suite with 869 automated tests:
...
- Applications view model (8 tests) - validates grouped pipeline statuses, note-only entries, template note appending, and status-menu mappings
...
- Report generation (50 tests) - validates HTML/Markdown output, clipboard UI, status tracking, shortlist state, keyboard navigation, compact/detail view, match summaries, filtered-out explanations, grouped skill callouts, large-report row caps, mobile layout, accessibility
...
- GUI onboarding (20 tests) - validates first-run demo report access, welcome/search feedback handling, Search tab profile-readiness guidance, profile-form field hints, profile-aware zero-result guidance wiring, non-blocking recent-search recording, recent-search reapply controls, named saved-search controls, Applications next-action queue wiring, and run metadata recording

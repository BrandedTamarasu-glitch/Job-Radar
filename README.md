...
- GUI onboarding (30 tests) - validates first-run demo report access, welcome/search feedback handling, Search tab profile-readiness guidance, Profile-tab dashboard next steps, profile-form field hints, profile-aware zero-result guidance wiring, non-blocking recent-search recording, recent-search reapply controls, recent and saved search comparison insight text, review queue summaries, Applications follow-up filters/next-action queue/quick complete/direct edit wiring, and run metadata recording

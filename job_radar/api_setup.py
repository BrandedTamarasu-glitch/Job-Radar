...
and app-data credential writes to keep secrets out of launch directories.
"""
...
from job_radar.api_config import ApiCredentialError, save_api_credentials
...
def setup_apis():
    ...
    and atomically writes the app-data credential file. Uses questionary for
    ...
    Notes
    -----
    App-data credential writes are atomic and use owner-only permissions where
    supported. KeyboardInterrupt (Ctrl+C) is handled gracefully with cleanup message.
    """
    print("\n" + "=" * 60)
    print("API Key Setup")
    print("=" * 60)
    ...
    try:
        env_path = save_api_credentials(credentials)

...
<task type="auto">
  ...
  <action>
    ...
    Note: Both `"kubernetes"` and `"k8s"` are added as separate entries so that lookup works in BOTH directions. When a user has "kubernetes" in their profile, it finds "k8s" as a variant. When a user has "k8s" in their profile, it finds "kubernetes" as a variant. The normalized keys are `"kubernetes"` and `"k8s"` respectively, so there is no collision.
  ...
  <verify>
    ...
    # FUZZ-05: New variants work -- forward direction
    ...
    print('PASS: all new variants work (forward)')

    # FUZZ-06: Reverse direction -- k8s looks up kubernetes
    assert _skill_in_text('k8s', 'kubernetes orchestration'), 'k8s should match kubernetes (reverse lookup)'
    assert _skill_in_text('k8s', 'managing kubernetes clusters'), 'k8s should match kubernetes in longer text'
    print('PASS: k8s -> kubernetes reverse direction works')
  ...
  <done>
    - 16 new tech variant entries added to `_SKILL_VARIANTS` (15 forward + 1 reverse for k8s)
    - Bidirectional kubernetes/k8s matching works: both "kubernetes" and "k8s" as profile skills find each other in job text
...
# === FUZZ-06: Reverse direction (k8s -> kubernetes) ===
check(_skill_in_text('k8s', 'kubernetes orchestration'), 'k8s -> kubernetes (reverse)')
check(_skill_in_text('k8s', 'managing kubernetes clusters'), 'k8s -> kubernetes in context')
...
6. 16 new tech variant entries added and functional (15 forward + 1 reverse k8s)
7. `_skill_in_text("k8s", "kubernetes orchestration")` returns True -- bidirectional matching
8. No key collisions in `_SKILL_VARIANTS_NORMALIZED`
9. Module imports with zero errors
</success_criteria>

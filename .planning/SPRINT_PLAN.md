...
Status: in progress. The source registry is in place, `fetch_all()` no longer uses long dispatch branches, source display/progress accounting is normalized, and query failures are summarized for CLI/reporting consumers. Generated Markdown/HTML reports now show partial source warnings from both CLI and GUI searches. Remaining work: deeper module extraction and richer in-app GUI surfacing for partial source failures.

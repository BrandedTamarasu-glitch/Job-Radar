...
Status: in progress. Portable app-data export bundles now include profile, config, saved searches, review state, tracker data, and a manifest while safely skipping missing files. Settings now exposes an Export App Data action that writes the bundle to the results directory.

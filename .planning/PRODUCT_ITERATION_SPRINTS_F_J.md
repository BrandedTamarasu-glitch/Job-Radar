...
Status: complete. Backend storage for recent and named saved searches is in place; successful GUI searches record recent configs and result metadata without blocking search execution; the Search tab can save and reapply saved/recent searches; and saved/recent searches show last-run summaries with previous-run result deltas.
...
Status: complete. Tracker-level next-action queueing now ranks application follow-ups across statuses, prioritizing overdue and due actions before dated future actions and unscheduled reminders. The Applications tab now surfaces the top follow-ups above grouped pipeline statuses with compact due-date labels. Application status/detail changes now record a compact per-entry timeline, CSV export includes timeline summaries, and CSV import/merge helpers restore portable application status data without clobbering newer local edits. Reusable note templates now cover follow-up, recruiter response, and cover-letter prep.

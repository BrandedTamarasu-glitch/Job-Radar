...
- `job_radar/search_pipeline.py` now contains shared search filter parsing, profile preference, freshness/date resolution, raw-result filtering, scoring/dealbreaker sorting, and post-score filtering helpers for GUI/CLI reuse; the CLI now uses the shared scorer, raw-result date filter, and composable post-score filters.
...
- Latest validation: `1132 passed, 8 skipped`.

...
- Rework Applications row actions so status, edits, and templates do not clip. Completed locally: Applications header actions, follow-up queue actions, and per-row status/edit/template controls now use grid-based wrapped rows.
- Improve report offline status feedback and status-sync messaging. Completed locally: report controls and status feedback clarify offline report use and manual JSON export/import into Applications.
- Review copy for macOS portable install paths, source counts, and feature claims. Completed locally: README, FAQ, and distribution README now match current source registries, status import/export behavior, macOS portable paths, and source-extension architecture.
...
- Latest Sprint 4 slice validation: `pytest tests/test_gui_onboarding.py tests/test_applications_view_model.py` (`87 passed`), CustomTkinter construction smoke at `700x500`, `900x600`, and widget scales `1.0`/`1.25`, and full `pytest tests/` (`1205 passed, 8 skipped`).
